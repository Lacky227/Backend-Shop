...
public class UserFilter implements GatewayFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (!request.getHeaders().containsKey("X-User-Role")) {
            log.warn("User access denied (missing X-User-Role header) for: {}", request.getPath());
            return onError(exchange, HttpStatus.UNAUTHORIZED);
        }
        String role = request.getHeaders().getFirst("X-User-Role");
        if (role == null || !role.equalsIgnoreCase("ROLE_USER")) {
            log.warn("User access denied (role is '{}') for: {}", role, request.getPath());
            return onError(exchange, HttpStatus.FORBIDDEN);
        }
        return chain.filter(exchange);
    }
    private Mono<Void> onError(ServerWebExchange exchange, HttpStatus status) {
        log.warn("User filter blocked request to: {}", exchange.getRequest().getPath());
